...
severity_filter = st.sidebar.selectbox(
    "Minimum Severity",
    options=["All", "High only", "Medium and above"]
)
...
with col2:
    st.metric("Anomalies Detected", f"{len(flagged):,}", delta=f"{len(flagged)/len(df)*100:.1f}% of total")
with col3:
    st.metric("High Severity Cases", f"{len(flagged[flagged['anomaly_score'] < -0.25]):,}")
with col4:
    st.metric("Est. Fraud Exposure", f"${flagged['total_cost'].sum()/1e6:.1f}M")
...
with col_left:
    ...
    fig1 = px.bar(fraud_counts, x='Fraud Type', y='Count',
                  color='Fraud Type',
                  color_discrete_map={
                      'cost_fraud': '#E74C3C',
                      'volume_fraud': '#E67E22',
...
with col_right:
    ...
    fig2 = px.bar(state_counts, x='Count', y='State',
...
filtered = filtered.sort_values('anomaly_score').head(50)
...
if selected_id:
    ...
    with col_a:
        ...
        st.metric("Fraud Type", case['fraud_type'])
    with col_b:
