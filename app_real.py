...
@st.cache_data
def load_data():
    return pd.read_csv("pharmaguard_results.csv")
...
def generate_explanation(row, regulation, regulation_source):
    # Load API key fresh every call — avoids Streamlit caching issues
...
# ── Safety disclaimer ─────────────────────────────────────────────────────────
st.warning(
    "⚠️ **PharmaGuard AI is a decision support tool.** "
    "All flags require independent human review. "
    "No disciplinary or legal action should be initiated based solely on AI-generated alerts."
)
...
with col1: st.metric("Total Prescriptions Analysed", f"{len(df):,}")
with col2: st.metric("Anomalies Detected", f"{len(flagged):,}",
                     delta=f"{len(flagged)/len(df)*100:.1f}% of total")
with col3: st.metric("High Severity Cases",
                     f"{len(flagged[flagged['anomaly_score'] < -0.25]):,}")
with col4: st.metric("Est. Fraud Exposure",
...
with col_left:
    ...
    fig1 = px.bar(
        fraud_counts, x='Fraud Type', y='Count', color='Fraud Type',
        color_discrete_map={
            'cost_fraud':   '#E74C3C',
            'volume_fraud': '#E67E22',
            'off_label':    '#F39C12'
    ...
    fig1.update_layout(showlegend=False, height=300, font=dict(family="Open Sans"))
...
st.dataframe(
    filtered[display_cols].rename(columns={
        'prescriber_id': 'Prescriber ID', 'generic_name': 'Drug',
        'specialty':     'Specialty',     'state':        'State',
        'total_claims':  'Total Claims',  'total_cost':   'Total Cost ($)',
        'fraud_type':    'Fraud Type',    'severity':     'Severity',
...
if len(filtered) == 0:
    ...
    if selected_id:
        ...
        if st.button("🤖 Generate AI Explanation", type="primary"):
            ...
            st.caption(
                "Complete both fields below before recording your decision. "
                "Your independent judgement is required — AI outputs are advisory only."
            )

            violation_type = st.selectbox(
                "Select violation type *",
                ["-- Select --", "Off-Label Use", "Volume Fraud",
                 "Reimbursement Fraud", "Insufficient Evidence"]
            )
            ...
            if not form_complete:
                st.info("Select a violation type and write at least 10 words to enable decision buttons.")